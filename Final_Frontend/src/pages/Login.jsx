...
import { DJANGO_BASE_URL, authApi } from '../utils/api';

const GOOGLE_CLIENT_ID = import.meta.env.VITE_GOOGLE_CLIENT_ID;

function GoogleSignInButton({ onDone }) {
  // Only render this component when Google client id is configured and provider is present
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { access, refresh, display_name, email } = res;
  ...
  const handleGoogleDone = (data) => {
    const { access, refresh, display_name, email } = data || {};
    if (access) localStorage.setItem('accessToken', access);
    if (refresh) localStorage.setItem('refreshToken', refresh);
    if (display_name) localStorage.setItem('displayName', display_name);
    if (email) localStorage.setItem('userEmail', email);
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      <div className="max-w-6xl w-full flex flex-col md:flex-row rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="w-full md:w-2/5 bg-gradient-to-br from-primary to-accent p-12 text-white flex flex-col justify-center">
          ...
          <Link to="/signup" className="mt-10 inline-block w-full">
            <button className="w-full py-3 px-4 rounded-lg text-gray-600 bg-white hover:bg-gray-50 transition-all duration-300 font-medium shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
              SIGN UP
            </button>
          </Link>
        </div>
        ...
        <div className="w-full md:w-3/5 bg-card p-12 flex flex-col justify-center">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-semibold text-foreground">Sign in to PactAI</h2>
          </div>

          {/* Social Login Buttons */}
          <div className="space-y-4 mb-6">
            {GOOGLE_CLIENT_ID ? (
              <GoogleSignInButton onDone={handleGoogleDone} />
            ) : (
              <button
                disabled
                title="Google login not configured"
                className="w-full flex items-center justify-center px-4 py-3 border border-border rounded-lg bg-card text-foreground opacity-60 cursor-not-allowed"
              >
                Continue with Google
              </button>
            )}
