#!/usr/bin/env python3
"""
Test script to verify all service connections are working properly.
"""
...
def test_cors(service_name, url):
    ...
    try:
        ...
        if 'Access-Control-Allow-Origin' in response.headers:
            print(f"✅ {service_name}: CORS configured correctly")
            return True
        else:
...
def main():
    print("=" * 60)
    print("GenAI Legal - Service Connection Test")
    ...
    if fastapi_ok:
        try:
            response = requests.get(f"{FASTAPI_URL}/", timeout=10)
            if response.status_code == 200:
                data = response.json()
